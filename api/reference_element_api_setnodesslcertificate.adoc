---
permalink: api/reference_element_api_setnodesslcertificate.html 
sidebar: sidebar 
keywords: ssl,certificate,set,setting,private,key,setnodesslcertificate 
summary: Mit der Methode SetNodeSSLCertificate können Sie ein Benutzer-SSL-Zertifikat und einen privaten Schlüssel für den Verwaltungsknoten festlegen. 
---
= SetNodeSSLCertificate
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Sie können die `SetNodeSSLCertificate` Methode zum Festlegen eines Benutzer-SSL-Zertifikats und eines privaten Schlüssels für den Verwaltungsknoten.


NOTE: Nach der Verwendung der API muss der Management-Knoten neu gestartet werden.



== Parameter


NOTE: Sie müssen diese Methode auf dem Management-Knoten aufrufen. Beispiel:

[listing]
----
https://<management node IP>:442/json-rpc/10.0
----
Diese Methode hat die folgenden Eingabeparameter:

|===
| Name | Beschreibung | Typ | Standardwert | Erforderlich 


 a| 
Zertifikat
 a| 
Die PEM-kodierte Textversion des Zertifikats.*Hinweis:* Beim Festlegen eines Knoten- oder Clusterzertifikats muss das Zertifikat die Erweiterung extendedKeyUsage für serverAuth enthalten.  Diese Erweiterung ermöglicht die fehlerfreie Verwendung des Zertifikats auf gängigen Betriebssystemen und Browsern.  Wenn die Erweiterung nicht vorhanden ist, lehnt die API das Zertifikat als ungültig ab.
 a| 
Schnur
 a| 
Keine
 a| 
Ja



 a| 
privater Schlüssel
 a| 
Die PEM-kodierte Textversion des privaten Schlüssels.
 a| 
Schnur
 a| 
Keine
 a| 
Ja

|===


== Rückgabewerte

Diese Methode hat keinen Rückgabewert.



== Anforderungsbeispiel

Anfragen für diese Methode ähneln dem folgenden Beispiel:

[listing]
----
{
    "method" : "SetNodeSSLCertificate",
    "params" : {
        "privateKey": "-----BEGIN RSA PRIVATE KEY-----\\nInDXvTgXMdMoet4BKnftelrXFKHgGqXJoczq4JWzGSIHNgvkrH60\n-----END RSA PRIVATE KEY-----\n",
        "certificate": "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n"
    },
    "id" : 2
}
----


== Antwortbeispiel

Diese Methode liefert eine Antwort, die dem folgenden Beispiel ähnelt:

[listing]
----
{
    "id" : 2,
    "result" : {}
}
----